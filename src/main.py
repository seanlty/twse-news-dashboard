...
def find_range_cache_file(data_dir: Path | None = None) -> Path | None:
    ...
    candidates = list(search_dir.glob("material_info_*.json"))
    range_output_path = default_range_output_path()
    if range_output_path.exists():
        candidates.append(range_output_path)
    if not candidates:
        return None
...
def seed_persistent_cache_files(
    ...
    source_files = list(source_raw_dir.glob("material_info_*.json"))
    monthly_seed = source_raw_dir / "monthly_revenue_latest.json"
    if monthly_seed.exists():
        source_files.append(monthly_seed)
