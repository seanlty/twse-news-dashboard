...
11. When `FINLAB_TOKEN` is set, the monthly revenue update enriches the newest displayed month with `EPS(估)`, `前季EPS`, and `EPS季增率(估)` using FinLab monthly revenue, previous-quarter EPS, previous-quarter capital, previous-quarter net margin, and `company_basic_info:普通股每股面額`. The par-value field replaces the old fixed NT$10 multiplier so split or non-NT$10 par-value stocks are adjusted correctly. FinLab input data is cached for `TWSE_DASHBOARD_FINLAB_CACHE_TTL_SECONDS`, defaulting to one day.
